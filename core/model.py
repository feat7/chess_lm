...
class BaseHFGPT(nn.Module):
    def __init__(self, config):
        ...
        self.n_params = sum(dict((p.data_ptr(), p.numel())
    ...
    def forward(self, input_ids, value_targets=None, loss=None, **gptkwargs):
        ...
        if loss is not None and value_targets is not None:
            ...
            loss_value = (values[:, :-1].contiguous().view(-1) -
                          value_targets[:, 1:].contiguous().view(-1)) ** 2
        ...
        with SummaryWriter(log_dir=config.tb_path, flush_secs=20) as tb:
            def run_epoch(split, _gs=None):
                ...
                num_batches = len(data) // config.batch_size + \
                    int(len(data) % config.batch_size != 0)
                ...
                for it, d in zip(pbar, dl):
                    ...
                    if is_train:
                        pbar.set_description(
                            f"[TRAIN] GS: {_gs}, IT: {it}, Loss: {round(_l, 5)}")
                    else:
                    ...
                    if is_train:
                        ...
                        tb.add_scalar("loss/loss_total", loss_total.item(),
                                      global_step=_gs, walltime=time.time())
                        tb.add_scalar("loss/policy", loss_policy.item(),
                                      global_step=_gs, walltime=time.time())
                        tb.add_scalar("loss/value", loss_value.item(),
                                      global_step=_gs, walltime=time.time())
...
class Evaluator():
    ...
    def eval(self):
        ...
        num_batches = len(data) // self.batch_size + \
            int(len(data) % self.batch_size != 0)
        pbar = trange(num_batches, ncols=100)
    ...
    def __iter__(self):
        ...
        with open(config.lm, "r") as flm, open(config.rf, "r") as fres:
            ...
            for lm, game_res in zip(flm, fres):
                ...
                # first will always generate 0
                results.extend([0] + res.tolist())
...
class ChessDataInMemory(Dataset):
    def __init__(self, config):
        ...
        self.lms = np.array(
            lms[:-(len(lms) % config.maxlen)]).reshape(-1, config.maxlen)
        self.results = np.array(
            results[:-(len(results) % config.maxlen)]).reshape(-1, config.maxlen)
